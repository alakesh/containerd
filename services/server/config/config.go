...
func (cfg *OpenTelemetryConfig) Validate() error {
	...
	case ExporterTypeOtlp:
		...
		if traceSamplingRatio < 0.1 || traceSamplingRatio > 0.99 {
			return errors.Wrapf(errdefs.ErrInvalidArgument, "trace sampling ratio out of range: %v", traceSamplingRatio)
		}
		return nil
	default:
